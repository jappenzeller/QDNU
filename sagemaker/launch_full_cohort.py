...
"""
================================================================================
PROMPT 018 - FULL COHORT SAGEMAKER LAUNCHER
================================================================================

Launch full cohort polarity analysis on AWS SageMaker.

This launcher uses train_full_cohort.py which includes:
- Validation gate (7 reference subjects, 0.76 AUC threshold)
- Tier 2: Correlation eigenvalues + RandomForest
- Tier 3: Riemannian TangentSpace + LDA
- Per-subject polarity calibration analysis

Usage:
    python sagemaker/launch_full_cohort.py --action upload    # Upload data to S3
    python sagemaker/launch_full_cohort.py --action launch    # Launch training job
    python sagemaker/launch_full_cohort.py --action status    # Check job status
    python sagemaker/launch_full_cohort.py --action download  # Download results
    python sagemaker/launch_full_cohort.py --action full      # Full pipeline
...
S3_OUTPUT_PREFIX = "chbmit/full-cohort-output"
S3_CODE_PREFIX = "chbmit/full-cohort-code"

# Local paths
LOCAL_DATA_DIR = Path("H:/Data/PythonDNU/EEG/chbmit")
LOCAL_OUTPUT_DIR = Path(__file__).parent.parent / "results" / "full_cohort"

# SageMaker Configuration
SAGEMAKER_ROLE_NAME = f"{PROJECT_NAME}-SageMakerRole"
TRAINING_INSTANCE_TYPE = "ml.m5.4xlarge"  # 16 vCPU, 64 GB RAM (larger for full cohort)
...
def upload_data(s3_client, force: bool = False):
    ...
    subjects = sorted([d for d in LOCAL_DATA_DIR.iterdir()
...
def create_source_archive(s3_client) -> str:
    ...
    with tarfile.open(archive_path, 'w:gz') as tar:
        # Include the full cohort training script
        tar.add(code_dir / 'train_full_cohort.py', arcname='train_full_cohort.py')
...
def launch_training_job(sagemaker_client, iam_client, s3_client) -> str:
    ...
    job_name = f"full-cohort-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    training_params = {
        ...
        'HyperParameters': {
            'sagemaker_program': '"train_full_cohort.py"',
    ...
    job_info = {
        ...
        'prompt': 'PROMPT 018 - Full Cohort Polarity Analysis',
    }
...
def main():
    parser = argparse.ArgumentParser(description="Full Cohort SageMaker Launcher")
    ...
    if args.action == 'upload':
        ...
        print("=" * 80)
        print("PROMPT 018 - FULL COHORT SAGEMAKER PIPELINE")
        print("=" * 80)
