...
Scheduled for March 22, 2026+ or subsequent window when credits reset.

Backend: IBM Heron r2 (ibm_torino) - no simulator
Encoding: PLV_theta_alpha (segment-level, NOT sub-windows)
Channels: 16 (33 qubits)
Shots: 1024
Cost estimate: ~9s/subject (7 subjects = ~63s total)

Output: results/projection/hardware_scaling.json (updates ch16 section)
================================================================================
"""
...
def load_channels() -> List[str]:
    """Load CH16 channel list from classical scaling results."""
    if CLASSICAL_SCALING.exists():
        with open(CLASSICAL_SCALING, 'r') as f:
            data = json.load(f)
        if 'ch16' in data:
            return data['ch16']['channels']
...
def run_hardware_loso(channels: List[str], subjects: List[str], backend) -> Dict:
    """Run LOSO CV on IBM hardware for CH16 using segment-level PLV."""

    n_channels = len(channels)
    n_qubits = 2 * n_channels + 1

    logger.info(f"Running CH16 hardware LOSO: {n_channels} channels, {n_qubits} qubits")
...
def main():
    ...
    logger.info(f"CH16 channels: {channels}")
    ...
    logger.info("\nRunning CH16 LOSO CV on hardware...")
    ...
    if sim_scaling.exists():
        ...
        sim_ch16_auc = sim_data.get('ch16', {}).get('overall_auc')

    gap_entry = {
        'channels': 16,
        'qubits': n_qubits,
        'sim_auc': sim_ch16_auc,
        'hw_auc': result['overall_auc'],
        'gap': (sim_ch16_auc - result['overall_auc']) if sim_ch16_auc else None,
        'encoding_match': True,
        'note': 'CH16 simulation was infeasible (128GB RAM required)'
    }

    # Update or append gap table entry
    gap_table = existing.get('gap_table', [])
    ch16_idx = next((i for i, g in enumerate(gap_table) if g.get('channels') == 16), None)
    if ch16_idx is not None:
        gap_table[ch16_idx] = gap_entry
    else:
    ...
    if sim_ch16_auc:
        gap = sim_ch16_auc - result['overall_auc']
        print(f"Simulation AUC: {sim_ch16_auc:.4f}")
        print(f"Hardware gap: {gap:.4f} ({100*gap/sim_ch16_auc:.1f}%)")
    else:
        print("Simulation AUC: N/A (128GB RAM required)")
